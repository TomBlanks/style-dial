// A "value set" is one complete set of token values (spec §9).
...
/** Token vars whose value differs from the defaults, in config order. */
export function changedKeys(config: TweakConfig, values: Values, defaults: Values): string[] {
  return config.tokens.map((t) => t.var).filter((k) => !sameValue(values[k], defaults[k]));
}
